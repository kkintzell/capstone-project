...
# function to extract 10 year historical data for constructing the portfolio
...
kNNdistplot(df.norm, k = 4)
abline(h = 0.45, lty = 2)

db <- fpc::dbscan(df.norm, eps = .9, MinPts = 4)
print(db)

fviz_cluster(db, data = df.norm, stand = FALSE,
...
# Table 
#table(dbr, iris$Species) 

# Run summary statistics
survey = cbind(cluster_features, cluster = db$cluster)
...
# Plot of annualized Returns vs. Volatility by cluster
cluster_features$Cluster = db$cluster
...
# CONSTRUCTING PORFOLIO USING TOP 3 STOCKS BASED ON SHARPE RATIO FROM EACH CLUSTER
Portfolio = rbind(SR_sorted_cluster1[1:3,],SR_sorted_cluster2[1:3,], SR_sorted_cluster3[1:3,])
Portfolio_Stocks = Portfolio$Symbols

# CONSTRUCTING PORFOLIO USING TOP 9 STOCKS BASED ON SHARPE RATIO (change)
Portfolio_ClusterX = SR_sorted_cluster[1:9,]
...
#for cluster X - top 9 stocks in 7 year historical data
...
ggplot(df, aes(x = Date, y = value)) + 
  ...
  scale_color_manual(values = c("orange", "red"))  + xlab("Years")   + ylab("Cumulative Returns")  + ggtitle("DBSCAN Portfolio vs S&P500 performance")  + theme_ipsum()
